...
const create_ticket = async (ticket) => {
    const url = "https://newaccount1641277823164.freshdesk.com/api/v2/tickets";
    const response = await fetch(url, {
        method: 'POST',
        headers: {
            "Authorization": "Basic " + btoa("gTFwLYzsWC3a9pG4Vt" + ":x"),
...
sboxbtn.addEventListener("click", async () => {
    ...
    const url = `https://newaccount1641277823164.freshdesk.com/api/v2/tickets/${ticket_id}?include=requester`;
...
const update_ticket = async (ticket) => {
    const url = `https://newaccount1641277823164.freshdesk.com/api/v2/tickets/${ticket_no}`;
    const response = await fetch(url, {
        method: 'PUT',
        headers: {
            "Authorization": "Basic " + btoa("gTFwLYzsWC3a9pG4Vt" + ":x"),
...
const del_ticket = async (T_number) => {
    const url = `https://newaccount1641277823164.freshdesk.com/api/v2/tickets/${T_number}`;
    const response = await fetch(url, {
        method: 'DELETE',
        headers: {
            "Authorization": "Basic " + btoa("gTFwLYzsWC3a9pG4Vt" + ":x"),
...
function allticket() {
    document.getElementById("dnone").style.cssText = "display:none;"
    document.getElementById("dblock").style.cssText = 'display:block'
            
            document.getElementById("allticket_type").innerHTML = "Subject of the tickets"
